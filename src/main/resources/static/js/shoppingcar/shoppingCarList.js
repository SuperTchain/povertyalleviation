layui.use(['laypage', 'layer', 'table', 'form', 'element', 'laydate'], function () {
    ...
    var product_table = table.render({
        elem: '#shoppingCarList'//表格的id
        , height: 600//表格的高度
        , url: '/shoppingcar/findAllShoppingCars?id='+userId//获取列表的后台接口(异步的)
            ...
            case 'delete':
                ...
                if (data.length === 0) {
                    ...
                    layer.confirm("确认要删除吗，删除后不能恢复", {btn: ['确定', '取消'], title: "提示"}, function () {
                        //将选中的id数组传递到后台，删除
                        $.ajax({
                            url: "/shoppingcar/batchDelete",//后台删除的接口
                            type: "post",
                            data: {
                                //id数组
                                "ids": ids
    ...
    //监听行右边工具事件
    table.on('tool(shoppingCarTable)', function (obj) { //注：tool 是工具条事件名，test是table lay-filter="对应的值"
        ...
        if (layEvent === 'detail') {
            ...
            layer.confirm("确认要删除吗，删除后不能恢复", {btn: ['确定', '取消'], title: "提示"}, function (index) {
                ...
                //向服务端发送删除指令
                $.ajax({
                    url: "/shoppingcar/deleteById",//后台删除的接口
                    type: "post",
                    data: {
                        id: data.id,//要删除行的id
                        userId:userId
