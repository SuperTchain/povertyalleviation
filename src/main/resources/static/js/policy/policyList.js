layui.use(['laypage', 'layer', 'table', 'form', 'element', 'laydate'], function () {
    ...
    var product_table = table.render({
        elem: '#policyList'//表格的id
        , height: 600//表格的高度
        , url: '/policy/findAllPolicy'//获取视频列表的后台接口(异步的)
        ...
        , cols: [[ //表头
            ...
            , {field: 'id', title: '政策ID', sort: true}
            , {field: 'policyTitle', title: '政策标题'}
            , {field: 'publisher', title: '发布人',sort:true}
            , {field: 'source', title: '来源'}
            , {
                field: 'publishTime', title: '发布日期',sort:true
                , templet: function (d) {
                    return showTime(d.publishTime);
                }
            }
            //unresize:true设置为true，代表不能拖动，默认是false，都能拖动
            , {field: 'viewed', title: '浏览量',sort:true}
            , {field: 'policyKind', title: '政策类别', templet: '#transfor_policyKind'}
            , {fixed: 'right', width: 170, toolbar: '#barDemo'}
        ]]
    });
    //监听头工具栏事件
    //监听的table标签中的lay-filter的取值
    table.on('toolbar(policyTable)', function (obj) {
            ...
            case 'delete':
                ...
                if (data.length === 0) {
                    ...
                    layer.confirm("确认要删除吗，删除后不能恢复", {btn: ['确定', '取消'], title: "提示"}, function () {
                        //将选中的id数组传递到后台，删除
                        $.ajax({
                            url: "/policy/batchDelete",//后台删除的接口
                            type: "post",
                            data: {
                                //id数组
                                "ids": ids
    ...
    //监听行右边工具事件
    table.on('tool(policyTable)', function (obj) { //注：tool 是工具条事件名，test是table lay-filter="对应的值"
